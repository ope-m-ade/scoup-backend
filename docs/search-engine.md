...
| `academic/views/search.py` | `unified_search()` exposes `GET /api/search/?q=<query>` |
...
Semantic search finds results based on *meaning*, not just keyword presence. It requires an OpenAI API key.
